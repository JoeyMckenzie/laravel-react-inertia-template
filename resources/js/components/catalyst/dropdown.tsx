...
export function DropdownItem({
    ...
    const classes = clsx(
        className,
        // Base styles
        "group cursor-default rounded-lg px-3.5 py-2.5 sm:px-3 sm:py-1.5 focus:outline-none",
        // Text styles
        "text-left text-base/6 text-zinc-950 dark:text-white forced-colors:text-[CanvasText] sm:text-sm/6",
        // Focus
        "data-[focus]:bg-blue-500 data-[focus]:text-white",
        // Disabled state
        "data-[disabled]:opacity-50",
        // Forced colors mode
        "forced-color-adjust-none forced-colors:data-[focus]:bg-[Highlight] forced-colors:[&>[data-slot=icon]]:data-[focus]:text-[HighlightText] forced-colors:data-[focus]:text-[HighlightText]",
        ...
        "[&>[data-slot=icon]]:dark:text-zinc-400 [&>[data-slot=icon]]:data-[focus]:dark:text-white [&>[data-slot=icon]]:data-[focus]:text-white [&>[data-slot=icon]]:text-zinc-500",
        // Avatar
            ...
            {(Array.isArray(keys) ? keys : keys.split("")).map(
                (char, index) => (
                    <kbd
                        ...
                        key={index}
                        className={clsx([
                            "min-w-[2ch] text-center font-sans text-zinc-400 capitalize forced-colors:group-data-[focus]:text-[HighlightText] group-data-[focus]:text-white",
